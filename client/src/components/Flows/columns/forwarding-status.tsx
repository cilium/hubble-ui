...
export const header = (
  ...
  return headerRenderer({
    title: COLUMN_TITLE.FORWARDING_STATUS,
    props,
    resize,
    isLast: false
  });
...
export const cell = (props: TableCellProps) => {
  return cellRenderer({
    props,
    renderer: flow => {
      let title = flow.ref.forwardingStatus.toLowerCase();
      const forwarded = title === "forwarded";
      const rejected = title === "rejected";
      const className = forwarded
        ? css.forwarded
        : rejected
        ? css.dropped
        : css.other;
      title = title === "undefined_policy_decision" ? "transient drop" : title;
