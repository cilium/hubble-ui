import { GetControlStreamResponse } from '~backend/proto/ui/ui_pb';
...
describe('ControlStream', () => {
  ...
  test('external data handler not dropped after reconnect', async () => {
    ...
    const responses = Array(4)
      .fill(null)
      .map(() => {
        return new GetControlStreamResponse();
      });

    const streamFn = () => {
      const newStream = DumbResponseStream.new<GetControlStreamResponse>();
    ...
    const stream = new ControlStream(DumbResponseStream.new(), {
      streamFn,
    });
